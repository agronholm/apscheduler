...
class CronBaseField:
    ...
    def is_all(self):
        ...
        if (
            len(self.rules) == 1
            and isinstance(self.rules[0], CronAllRule)
            and not self.rules[0].step
        ):
            return True
        return False
        ...
        for expr in pattern.split(self.SEPARATOR):
            ...
            if not found:
                ...
                raise CronParserError(
                    expr, self.REAL_NAME, self.CRONTAB_INDEX, self.crontab, exceptions
                )
    ...
    def __repr__(self):
        ...
        return (
            f'{self.__class__.__name__}({", ".join(repr(rule) for rule in self.rules)})'
        )
