...
from ...utils.calendar import parse_weekday, resolve_day_name, get_weekday_in_month
from ...utils.constant import (
    WEEKDAYS_SUNDAY, WEEKDAYS_RE,
    WEEKDAY_OPTIONS_SHORT, WEEKDAY_OPTIONS_SHORT_RE,
    WEEKDAY_OPTIONS_LONG, WEEKDAY_OPTIONS_LONG_RE,
)
...
class CronDayOfWeekPositionRule(CronRangeRule):
    ...
    OPTIONS = {
        'short': WEEKDAY_OPTIONS_SHORT,
        'long': WEEKDAY_OPTIONS_LONG
    }
    ...
    def __init__(self, first, step, rule_id, min_val=0, max_val=9999):
        ...
        super(CronDayOfWeekPositionRule, self).__init__(
            first=first,
            step=step,
            min_val=min_val,
            max_val=max_val
    ...
    def parse(self, value, value_type):
        ...
        if value_type == 'first':
            ...
            try:
                return self.OPTIONS[self.rule_id].index(
                    str_lower_strip(value, '')
                ), False
    ...
    def _validate_step(self):
        ...
        if self.step > 6:
            raise ValueError(f'step value {self.prettify(self.step, "step")} is invalid')
        
    def next(self, date, field):

        weekdays = get_weekday_in_month(date.year, date.month, WEEKDAYS_SUNDAY[self.first])
