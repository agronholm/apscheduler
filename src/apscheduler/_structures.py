...
class Task:
    ...
    :ivar id: the unique identifier of this task
    :ivar func: the callable that is called when this task is run
    :ivar max_running_jobs: maximum number of instances of this task that are allowed
        to run concurrently
    :ivar misfire_grace_time: maximum number of seconds the run time of jobs created
...
class Schedule:
    ...
    :ivar id: the unique identifier of this schedule
    :ivar task_id: unique identifier of the task to be run on this schedule
    :ivar args: positional arguments to pass to the task callable
    :ivar kwargs: keyword arguments to pass to the task callable
    :ivar coalesce: if ``True``, only one job will be created when the schedule is
        due even if multiple run times have passed since the last processing of the
        schedule
    :ivar misfire_grace_time: maximum number of seconds the scheduled job's actual
        run time is allowed to be late, compared to the scheduled run time
    :ivar max_jitter: maximum number of seconds to randomly add to the scheduled
        time for each job created from this schedule
    :ivar tags: strings that can be used to categorize and filter the schedule and
        its derivative jobs
    :ivar conflict_policy: determines what to do if a schedule with the same ID
        already exists in the data store
    :ivar next_fire_time: the next time the task will be run
    :ivar last_fire_time: the last time the task was scheduled to run
    :ivar acquired_by: ID of the scheduler that has acquired this schedule for
        processing
    :ivar acquired_until: the time after which other schedulers are free to acquire the
...
class Job:
    ...
    :ivar id: autogenerated unique identifier of the job
    :ivar task_id: unique identifier of the task to be run
    :ivar args: positional arguments to pass to the task callable
    :ivar kwargs: keyword arguments to pass to the task callable
    :ivar schedule_id: unique identifier of the associated schedule
        (if the job was derived from a schedule)
    :ivar scheduled_fire_time: the time the job was scheduled to run at
        (if the job was derived from a schedule; includes jitter)
    :ivar jitter: the time that was randomly added to the calculated scheduled run time
        (if the job was derived from a schedule)
    :ivar start_deadline: if the job is started in the worker after this time, it is
        considered to be misfired and will be aborted
    :ivar tags: strings that can be used to categorize and filter the job
    :ivar created_at: the time at which the job was created
    :ivar started_at: the time at which the execution of the job was started
    :ivar acquired_by: the unique identifier of the worker that has acquired the job
        for execution
    :ivar acquired_until: the time after which other workers are free to acquire the
...
class JobInfo:
    ...
    :ivar job_id: the unique identifier of the job
    :ivar task_id: the unique identifier of the task that is being run
    :ivar schedule_id: the unique identifier of the schedule that the job was derived
        from (if any)
    :ivar jitter: the time that was randomly added to the calculated scheduled run time
        (if the job was derived from a schedule)
    :ivar start_deadline: if the job is started in the worker after this time, it is
...
class JobResult:
    ...
    :ivar job_id: the unique identifier of the job
    :ivar outcome: indicates how the job ended
    :ivar finished_at: the time when the job ended
    :ivar exception: the exception object if the job ended due to an exception being
        raised
    :ivar return_value: the return value from the task function (if the job ran to
...
class RetrySettings:
    ...
    :ivar stop: defines when to stop trying
    :ivar wait: defines how long to wait between attempts
    """
