...
class TestCronTrigger:
    ...
    def test_week_1(self, timezone):
        ...
        assert repr(trigger) == (
            "<CronTrigger (year='2009', month='2', week='8', "
            "timezone='Europe/Berlin')>"
        )
    ...
    def test_jitter_produces_differrent_valid_results(self, timezone):
        ...
        for _ in range(0, 100):
            ...
            assert (
                timedelta(seconds=25) <= (next_fire_time - now) <= timedelta(seconds=35)
            )
...
class TestIntervalTrigger:
    ...
    def test_within(self, trigger, timezone):
        ...
        assert (
            trigger.get_next_fire_time(None, now)
            == trigger.start_date + trigger.interval
        )
    ...
    def test_end_date(self, timezone):
        ...
        assert (
            trigger.get_next_fire_time(None, start_date + timedelta(minutes=6)) is None
        )
    ...
    def test_space_in_expr(self, timezone):
        ...
        assert (
            repr(trigger) == "<CronTrigger (day='1-2,4-7', timezone='Europe/Berlin')>"
        )
    ...
    def test_jitter_produces_different_valid_results(self, timezone):
        ...
        for _ in range(0, 100):
            ...
            results.add(next_fire_time)
            assert (
                timedelta(seconds=2) <= (next_fire_time - now) <= timedelta(seconds=8)
            )
        assert 1 < len(results)
